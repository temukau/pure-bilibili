...
import FollowBtn from "@/components/follow-btn.vue";
  ...
  <div class="item">
    <div class="item-header">
      <div class="item-owner-info">
        ...
        <p class="item-owner-name">{{ Time.timeStampToAgo(item.pubdate) }} · {{ item.owner.name }}</p>
      </div>
      <follow-btn :is-followed="item.is_followed"/>
    </div>
    <div class="item-cover-box">
      ...
      <div class="item-cover-video-info">
        <div></div>
        <div class="video-length">{{ Time.secondsToTime(item.duration) }}</div>
      </div>
    </div>
    <div class="item-info">
...
.item-owner-name {
  width: calc(100% - 40px - var(--padding-normal));
